...
class MY_Model extends CI_Model
{
    ...
    */
    //kiem tra su ton tai true la ton tai,nguoc lai fasle
    ...
    function get_rule($levelId = '')
    {
        if ($levelId == '')
            return false;
        $this->db->where('MA_CHUCVU', $levelId);
        $this->db->select('TEN_CHUCVU');
    ...
    //thuc hien lay du lieu chi tiet bang nao theo dieu kien gi
    public function getDetailInfo($table = '', $where = array())
    {
        if ($table == '')
            return false;
    ...
    public function count($input)
    {
        ...
        if ((isset($input['count'])) && $input['count']) {
            $this->db->select("" . $input['select'] . ",COUNT(" . $input['count'] . ") as SUM");
        }
