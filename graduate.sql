...
CREATE TABLE IF NOT EXISTS `graduate`.`giaodich` (
  ...
  `MA_KHACHHANG` INT(11) NOT NULL,
  `MA_HINHTHUC` INT(11) NOT NULL,
  `MA_NHANVIEN` INT(11) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `graduate`.`chitiet_kho` (
  ...
  `LUOTXEM` INT NULL,
  `MA_XUATXU` INT(11) NOT NULL,
  PRIMARY KEY (`MA_KHO`, `MA_LOAI_SANPHAM`, `MA_XUATXU`),
...
CREATE TABLE IF NOT EXISTS `graduate`.`sanpham` (
  ...
  `MA_KHO` INT(11) NOT NULL,
  `MA_LOAI_SANPHAM` INT(11) NOT NULL,
  PRIMARY KEY USING BTREE (`MA_SANPHAM`, `MA_KHO`, `MA_LOAI_SANPHAM`),
  INDEX `fk_sanpham_chitiet_kho1_idx` (`MA_KHO` ASC, `MA_LOAI_SANPHAM` ASC),
  CONSTRAINT `fk_sanpham_chitiet_kho1`
    FOREIGN KEY (`MA_KHO` , `MA_LOAI_SANPHAM`)
    REFERENCES `graduate`.`chitiet_kho` (`MA_KHO` , `MA_LOAI_SANPHAM`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
CREATE TABLE IF NOT EXISTS `graduate`.`chitiet_giaodich` (
  ...
  PRIMARY KEY (`MA_GIAODICH`, `MA_SANPHAM`, `MA_KHO`, `MA_LOAI_SANPHAM`),
  INDEX `fk_chitiet_giaodich_sanpham1_idx` (`MA_SANPHAM` ASC, `MA_KHO` ASC, `MA_LOAI_SANPHAM` ASC),
  ...
  CONSTRAINT `fk_chitiet_giaodich_sanpham1`
    FOREIGN KEY (`MA_SANPHAM` , `MA_KHO` , `MA_LOAI_SANPHAM`)
    REFERENCES `graduate`.`sanpham` (`MA_SANPHAM` , `MA_KHO` , `MA_LOAI_SANPHAM`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `graduate`.`chitiet_khuyenmai` (
  ...
  PRIMARY KEY (`MA_KHUYENMAI`, `MA_KHO`, `MA_LOAI_SANPHAM`),
  INDEX `fk_chitiet_khuyenmai_khuyenmai1_idx` (`MA_KHUYENMAI` ASC),
  INDEX `fk_chitiet_khuyenmai_chitiet_kho1_idx` (`MA_KHO` ASC, `MA_LOAI_SANPHAM` ASC),
...
CREATE TABLE IF NOT EXISTS `graduate`.`chitiet_nhap` (
  ...
  `chitiet_kho_MA_KHO` INT(11) NOT NULL,
  `chitiet_kho_MA_LOAI_SANPHAM` INT(11) NOT NULL,
  PRIMARY KEY (`MA_PHIEUNHAP`, `chitiet_kho_MA_KHO`, `chitiet_kho_MA_LOAI_SANPHAM`),
  INDEX `MA_PHIEUNHAP` (`MA_PHIEUNHAP` ASC),
  INDEX `fk_chitiet_nhap_chitiet_kho1_idx` (`chitiet_kho_MA_KHO` ASC, `chitiet_kho_MA_LOAI_SANPHAM` ASC),
  ...
  CONSTRAINT `fk_chitiet_nhap_chitiet_kho1`
    FOREIGN KEY (`chitiet_kho_MA_KHO` , `chitiet_kho_MA_LOAI_SANPHAM`)
    REFERENCES `graduate`.`chitiet_kho` (`MA_KHO` , `MA_LOAI_SANPHAM`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `graduate`.`dacdiem_kythuat` (
  ...
  PRIMARY KEY (`MA_SANPHAM`, `MA_KHO`, `MA_LOAI_SANPHAM`),
  CONSTRAINT `fk_dacdiem_kythuat_sanpham1`
    FOREIGN KEY (`MA_SANPHAM` , `MA_KHO` , `MA_LOAI_SANPHAM`)
    REFERENCES `graduate`.`sanpham` (`MA_SANPHAM` , `MA_KHO` , `MA_LOAI_SANPHAM`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `graduate`.`baohanh` (
  `MA_GIAODICH` INT(11) NOT NULL,
  `MA_SANPHAM` INT(11) NOT NULL,
  `LAN_BAOHANH` INT NOT NULL DEFAULT 0,
  `NGAY_TIEPNHAN` DATE NULL DEFAULT NULL,
  `NOIDUNG_BH` TEXT NULL DEFAULT NULL,
  `PHI_BH` INT NULL DEFAULT NULL,
  `MA_NHANVIEN` INT NULL DEFAULT NULL COMMENT 'nhân viên tiếp nhận bảo hành',
  ...
  PRIMARY KEY (`MA_GIAODICH`, `MA_SANPHAM`, `LAN_BAOHANH`),
  INDEX `fk_baohanh_chitiet_giaodich1_idx` (`MA_GIAODICH` ASC, `MA_SANPHAM` ASC),
  CONSTRAINT `fk_baohanh_nhanvien1`
    FOREIGN KEY (`MA_NHANVIEN`)
    REFERENCES `graduate`.`nhanvien` (`MA_NHANVIEN`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_baohanh_chitiet_giaodich1`
    FOREIGN KEY (`MA_GIAODICH`)
    REFERENCES `graduate`.`chitiet_giaodich` (`MA_GIAODICH`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
